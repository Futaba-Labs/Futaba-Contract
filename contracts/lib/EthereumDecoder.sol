...
library EthereumDecoder {
    ...
    function toBlockHeader(
        ...
        RLPReader.Iterator memory it = RLPReader
            .toRlpItem(rlpHeader)
            .iterator();
    ...
    function getReceiptRlpData(
        TransactionReceiptTrie memory receipt
    ) internal pure returns (bytes memory data) {
    ...
    function toReceipt(
        bytes memory data
    ) internal pure returns (TransactionReceiptTrie memory receipt) {
        RLPReader.Iterator memory it = RLPReader.toRlpItem(data).iterator();
